...
void rec(char *inp)
{
    ...
    while (ent != NULL) {
        ...
        data[cnt].lasttime =
            (char *)malloc(sizeof(char) * strlen(ctime(&buf.st_atime)) + 2);
        ...
        data[cnt].uname =
            (char *)malloc(sizeof(char) * strlen(getpwuid(buf.st_uid)->pw_name) + 2);
        strcpy(data[cnt].uname, getpwuid(buf.st_uid)->pw_name);
        data[cnt].gname =
            (char *)malloc(sizeof(char) * strlen(getgrgid(buf.st_gid)->gr_name) + 2);
    ...
    while (optcnt < cnt) {
        if (data[optcnt].isdir == 'd' && strcmp(data[optcnt].name, ".") &&
            ...
            char *next = (char *)malloc(sizeof(char) * strlen(inp) + 1000);
            strcpy(next, inp);
            printf("%s\n", data[optcnt].name);
